...
$(document).ready(function() {
                ...
                function configureLocationChanged() {
                    ...
                    var isCallBackToken = url.match('http://niths.no/callback%23state=/profile&access_token=.+&token_type=Bearer&expires_in=3600') != null;     

                        // Triggered if the app is denied access
                    if (url == callbackURL + '#error=access_denied' + stateURLFragment) {
                        alert('Kunne ikke fa tilgang');
                        window.plugins.childBrowser.close();
                        resetUserValues();
                  // Triggered if a token is received
                    } else if (isCallBackToken) {
                        console.log("Callback okey");    
                        window.plugins.childBrowser.close();
